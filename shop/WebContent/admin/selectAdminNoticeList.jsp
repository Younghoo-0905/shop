...
<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css">
	...
	<div class="container-fluid">
		...
		%>			
				<li class="page-item"><a class="page-link" href="<%=request.getContextPath() %>/admin/selectAdminNoticeList.jsp?currentPage=<%=(pagingNum * 10) %>">이전</a></li>
		<%
			}				
		...
		%>
					<li class="page-item active"><a class="page-link" href="<%=request.getContextPath() %>/admin/selectAdminNoticeList.jsp?currentPage=<%=i + (pagingNum * 10) %>"><%=i + (pagingNum * 10) %></a></li>
		<%
				} else {									//	currentPage 아닌 링크버튼
		%>					
					<li class="page-item"><a class="page-link" href="<%=request.getContextPath() %>/admin/selectAdminNoticeList.jsp?currentPage=<%=i + (pagingNum * 10) %>"><%=i + (pagingNum * 10) %></a></li>
		<%
				}
		...
		%>
				<li class="page-item"><a class="page-link" href="<%=request.getContextPath() %>/admin/selectAdminNoticeList.jsp?currentPage=<%=(pagingNum * 10) + 11 %>">다음</a></li>					
		<%
			}
		%>							
			</ul>	
